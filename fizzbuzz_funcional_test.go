...
func TestFizzBuzzFunctional(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got := gofizbuzz.FizzBuzzFunctional(tc.n)
			assertEquals(t, tc.want, got)
		})
	}
}

func BenchmarkFizzBuzzFunctional(b *testing.B) {
	defer quiet()()

	for n := 0; n < b.N; n++ {
		gofizbuzz.FizzBuzzFunctional(n)
	}
}
